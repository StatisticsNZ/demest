            ...
            if (update.series[l]) {
                ...
                for (i in seq.int(from = K - 1L, to = 0L)) {
                    if (!has.level) {
                        ...
                        if ((i == 0L) && is.infinite(DC.inv[[1L]][1L])) {
                            C.star.22 <- (C[[1L]][4L]
                                - C[[1L]][4L]^2 * (R.inv[1L] + phi * (R.inv[2L] + R.inv[3L]) + phi^2 * R.inv[4L]))
                            sqrt.C.star <- matrix(c(0, 0, 0, sqrt(C.star.22)), nrow = 2L)
                        }
                        else {
...
updateAlphaDLMNoTrend <- function(prior, betaTilde, useC = FALSE) {
    ...
    if (useC) {
        ...
        K <- prior@K@.Data
        L <- prior@L@.Data
        update.series <- prior@updateSeriesDLM # logical vector length L ## NEW
