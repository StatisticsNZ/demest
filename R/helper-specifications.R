...
checkAndTidyJump <- function(jump) {
    if (is.null(jump)) {
        ...
        if (jump <= 0)
            stop(gettextf("'%s' is non-positive", "jump"))
...
checkAndTidyUseHMC <- function(useHMC) {
    if (is.null(useHMC)) {
        ...
        ## 'useHMC' has length 1
        if (!identical(length(useHMC), 1L))
            stop(gettextf("'%s' does not have length %d", "useHMC", 1L))
        ## 'useHMC' is not missing
        if (is.na(useHMC))
            stop(gettextf("'%s' is missing", "useHMC"))
        ## 'useHMC' is logical
        if (!is.logical(useHMC))
            stop(gettextf("'%s' does not have type \"%s\"",
                          "useHMC", "logical"))
        methods::new("LogicalFlag", useHMC)
    }
}

## HAS_TESTS
checkAndTidySizeStep <- function(sizeStep) {
    if (is.null(sizeStep)) {
        methods::new("Scale", 0.1)
    }
    else {        
        ## 'sizeStep' has length 1
        if (!identical(length(sizeStep), 1L))
            stop(gettextf("'%s' does not have length %d", "sizeStep", 1L))
        ## 'sizeStep' is not missing
        if (is.na(sizeStep))
            stop(gettextf("'%s' is missing", "sizeStep"))
        ## 'sizeStep' is numeric
        if (!is.numeric(sizeStep))
            stop(gettextf("'%s' is not numeric",
                          "sizeStep"))
        sizeStep <- as.double(sizeStep)
        ## 'sizeStep' is positive
        if (sizeStep <= 0)
            stop(gettextf("'%s' is non-positive", "sizeStep"))
        methods::new("Scale", sizeStep)
    }
}

## HAS_TESTS
checkAndTidyNStep <- function(nStep) {
    if (is.null(nStep)) {
        methods::new("Length", 10L)
    }
    else {        
        ## 'nStep' has length 1
        if (!identical(length(nStep), 1L))
            stop(gettextf("'%s' does not have length %d", "nStep", 1L))
        ## 'nStep' is not missing
        if (is.na(nStep))
            stop(gettextf("'%s' is missing", "nStep"))
        ## 'nStep' is numeric
        if (!is.numeric(nStep))
            stop(gettextf("'%s' is not numeric",
                          "nStep"))
        ## 'nStep' is a whole number
        if (!isTRUE(all.equal(as.integer(nStep), nStep)))
            stop(gettextf("'%s' is not an integer",
                          "nStep"))
        nStep <- as.integer(nStep)
        ## 'nStep' is positive
        if (nStep <= 0L)
            stop(gettextf("'%s' is non-positive", "nStep"))
        methods::new("Length", nStep)
    }
}
