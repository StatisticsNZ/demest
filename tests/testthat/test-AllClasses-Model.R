...
test_that("can create a valid object of class PoissonVaryingNotUseExp", {
    ...
    x <- new("PoissonVaryingNotUseExp",
             ...
             betasOld = list(5, rnorm(5), rnorm(4)),
             meansBetas = list(0, rep(0, 5), rep(0, 4)),
             variancesBetas = list(0, rep(0, 5), rep(0, 4)),
             gradientBetas = list(0, rep(0, 5), rep(0, 4)),
             momentumBetas = list(0, rep(0, 5), rep(0, 4)),
             betaEqualsMean = rep(FALSE, 3),
             useHMCToUpdateBeta = c(FALSE, TRUE, TRUE),
    ...
    x <- new("PoissonVaryingNotUseExp",
             ...
             betasOld = list(5),
             meansBetas = list(0),
             variancesBetas = list(0),
             momentumBetas = list(0),
             gradientBetas = list(0),
             betaEqualsMean = FALSE,
             useHMCToUpdateBeta = FALSE,
...
test_that("validity tests for PoissonVaryingNotUseExp inherited from Betas work", {
    ...
    x.wrong <- x
    x.wrong@gradientBetas[[1]] <- as.integer(x.wrong@betas[[1]])
    expect_error(validObject(x.wrong),
                 "'gradientBetas' has elements not of type \"double\"")
    ## 'gradientBetas' does not have names
    x.wrong <- x
    names(x.wrong@gradientBetas) <- c("a", "b", "c")
    expect_error(validObject(x.wrong),
                 "'gradientBetas' has names")
    ## all elements of 'momentumBetas' have type "double"
    x.wrong <- x
    x.wrong@momentumBetas[[1]] <- as.integer(x.wrong@betas[[1]])
    expect_error(validObject(x.wrong),
                 "'momentumBetas' has elements not of type \"double\"")
    ## 'momentumBetas' does not have names
    x.wrong <- x
    names(x.wrong@momentumBetas) <- c("a", "b", "c")
    expect_error(validObject(x.wrong),
                 "'momentumBetas' has names")
    ...
    x.wrong@meansBetas[[2]] <- 0
    x.wrong@betasOld[[2]] <- 0
    x.wrong@variancesBetas[[2]] <- 0
    x.wrong@gradientBetas[[2]] <- 0
    x.wrong@momentumBetas[[2]] <- 0
    expect_error(validObject(x.wrong),
    ...
    x.wrong <- x
    x.wrong@gradientBetas <- x.wrong@gradientBetas[1:2]
    expect_error(validObject(x.wrong),
                 "'betas' and 'gradientBetas' have different lengths")
    ## corresponding elements of 'gradientBetas' and 'betas' have same length
    x.wrong <- x
    x.wrong@gradientBetas[[2]] <- x.wrong@gradientBetas[[2]][1:3]
    expect_error(validObject(x.wrong),
                 "element 2 of 'gradientBetas' and element 2 of 'betas' have different lengths")
    ## corresponding elements of 'gradientBetas' and 'betas' have same length
    x.wrong <- x
    x.wrong@gradientBetas[[2]][1] <- NA
    expect_error(validObject(x.wrong),
                 "element 2 of 'gradientBetas' and element 2 of 'betas' have different patterns of missingness")
    ## 'betas' and 'momentumBetas' have same length
    x.wrong <- x
    x.wrong@momentumBetas <- x.wrong@momentumBetas[1:2]
    expect_error(validObject(x.wrong),
                 "'betas' and 'momentumBetas' have different lengths")
    ## corresponding elements of 'momentumBetas' and 'betas' have same length
    x.wrong <- x
    x.wrong@momentumBetas[[2]] <- x.wrong@momentumBetas[[2]][1:3]
    expect_error(validObject(x.wrong),
                 "element 2 of 'momentumBetas' and element 2 of 'betas' have different lengths")
    ## corresponding elements of 'momentumBetas' and 'betas' have same length
    x.wrong <- x
    x.wrong@momentumBetas[[2]][1] <- NA
    expect_error(validObject(x.wrong),
                 "element 2 of 'momentumBetas' and element 2 of 'betas' have different patterns of missingness")
})
