...
test_that("initialCombinedCountsPredict creates object of class CombinedCountsBinomial from valid inputs", {
    initialCombinedCounts <- demest:::initialCombinedCounts
    ...
    expect_identical(length(x.pred@dataModels), 2L)
    x.pred <- initialCombinedCountsPredict(x.est,
                                           along = 3L,
                                           labels = NULL,
                                           n = 2L,
    ...
    expect_is(x.est, "CombinedCountsBinomial")
    expect_true(validObject(x.pred))
    expect_identical(length(x.pred@dataModels), 0L)
})
