...
setClass("ThetaTransformedMixin",
         slots = c(thetaTransformed = "numeric"),
         contains = "VIRTUAL",
         validity = function(object) {
             thetaTransformed <- object@thetaTransformed
             theta <- object@theta
             cell.in.lik <- oject@cellInLik
             ## 'thetaTransformed' has type "double"
             if (!is.double(thetaTransformed))
                 return(gettextf("'%s' does not have type \"%s\"",
                                 "thetaTransformed", "double"))
             ## 'thetaTransformed' has same length as 'theta'
             if (!identical(length(thetaTransformed), length(theta)))
                 return(gettextf("'%s' and '%s' have different lengths",
                                 "thetaTransformed", "theta"))
             ## 'thetaTransformed' missing values iff cell not in likelihood
             if (!identical(is.na(thetaTransformed), cell.in.lik))
                 stop(gettextf("'%s' and '%s' inconsistent",
                               "thetaTransformed", "cellInLik"))
             TRUE
         })
